...
/* SDIO streaming mode can deliver multi-packet bursts larger than the
 * single transport packet size (1536 bytes). Reserve a stable internal DMA
 * buffer up front so OTA traffic does not trigger reallocations in the hot path. */
#define STABLE_SDIO_STREAM_RX_BUFFER_SIZE 16384
...
static esp_err_t sdio_preallocate_double_buffers(void)
{
	int index;
	const uint32_t stable_size = STABLE_SDIO_STREAM_RX_BUFFER_SIZE;

	for (index = 0; index < 2; ++index)
	{
		if (double_buf.buffer[index].buf == NULL || double_buf.buffer[index].buf_size < stable_size)
		{
			if (double_buf.buffer[index].buf != NULL)
			{
				g_h.funcs->_h_free_align(double_buf.buffer[index].buf);
				double_buf.buffer[index].buf = NULL;
				double_buf.buffer[index].buf_size = 0;
			}

			double_buf.buffer[index].buf = (uint8_t*)g_h.funcs->_h_malloc_align(
				stable_size,
				HOSTED_MEM_ALIGNMENT_64);
			if (double_buf.buffer[index].buf == NULL)
			{
				ESP_LOGE(TAG,
					"Failed to preallocate SDIO RX double buffer %d (%u bytes)",
					index,
					(unsigned)stable_size);
				return ESP_ERR_NO_MEM;
			}

			double_buf.buffer[index].buf_size = stable_size;
			ESP_LOGI(TAG,
				"Preallocated SDIO RX double buffer %d with %u bytes",
				index,
...
void* bus_init_internal(void)
{
	...
	if (sdio_preallocate_double_buffers() != ESP_OK)
	{
		ESP_LOGE(TAG, "Unable to reserve stable SDIO RX buffers during init");
		assert(false);
	}
