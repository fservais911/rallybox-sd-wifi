...
static bool track_fetch_presigned_upload_url(const char* filename, char* out_url, size_t out_len);
static bool track_resolve_web_upload_url(const char* filename, char* out_url, size_t out_len);
...
static bool track_fetch_presigned_upload_url(const char* filename, char* out_url, size_t out_len)
{
  ...
  {
    ESP_LOGE(TAG, "Signer response did not contain a valid upload URL");
    goto cleanup;
  }
...
static bool track_resolve_web_upload_url(const char* filename, char* out_url, size_t out_len)
{
  ...
  return track_build_web_url(filename, out_url, out_len);
}
...
static void track_try_normalize_s3_upload_url(char* url, size_t url_len)
{
  ...
  if (!sigv4_parse_url_parts(url, host, sizeof(host), uri, sizeof(uri), query, sizeof(query)))
  {
    return;
  }
...
static esp_err_t track_upload_gpx_snapshot_to_url(gps_points_feed_t feed, const char* url, size_t* out_points, time_t* out_first_utc)
{
  ...
  char request_uri[640];
  char request_query[512];
  char request_path[1152];
...
esp_err_t gps_points_upload_web(gps_points_feed_t feed, const char* filename, size_t* out_points, time_t* out_first_utc)
{
  ...
  if (!track_resolve_web_upload_url(filename, url, sizeof(url)))
  {
    return ESP_ERR_INVALID_ARG;
  }

  return track_upload_gpx_snapshot_to_url(feed, url, out_points, out_first_utc);
}